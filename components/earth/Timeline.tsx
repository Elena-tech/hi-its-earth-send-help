...
export default function Timeline({ year, scenario, onYearChange, onScenarioChange }: Props) {
  ...
  return (
    <div style={{
      ...
      {/* Year labels */}
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "9px", color: "rgba(255,255,255,0.3)", letterSpacing: "0.1em", marginTop: "4px" }}>
        {[1850, 1900, 1950, 2000, 2024, 2050, 2100].map(y => (
          <span key={y} onClick={() => onYearChange(y)} style={{ cursor: "pointer", color: y === 2024 ? "rgba(255,255,255,0.6)" : "rgba(255,255,255,0.3)" }}>
            {y}
          </span>
        ))}
      </div>
